  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (raw.trimStart().startsWith('```')) {
      ...
      while (i < lines.length && !lines[i].trimStart().startsWith('```')) {
        result.push({ text: '  ' + lines[i], indent: 1, code: true })
        i++
      }
      continue
    }
    ...
    if (headingMatch) {
      ...
      result.push({ text: level === 1 ? '== ' + text + ' ==' : '-- ' + text, bold: true, level })
      continue
    }
    ...
    // Table row (GFM) — lines starting with |
    if (raw.trim().startsWith('|')) {
      // Separator row (|---|---|) — skip
      if (/^\|[\s\-:|]+\|$/.test(raw.trim())) {
        continue
      }
      const cells = raw.split('|').slice(1, -1).map(c => stripInline(c.trim())).filter(c => c)
      if (cells.length > 0) {
        const nextRaw = (lines[i + 1] || '').trim()
        const isHeader = /^\|[\s\-:|]+\|$/.test(nextRaw)
        result.push({ text: cells.join(' | '), table: true, header: isHeader })
        continue
      }
    }
